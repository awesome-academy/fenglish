		...
		<div class="col-md-8 col-sm-8 col-xs-12">
			<div class="col_box_baihoc_view col_box_test" id="test_random">
				<form id="testing_form"
					action="${pageContext.request.contextPath}/exercises/result"
					method="POST">
					...
					<div class="question alias_answer">
						<div class="myquestionarea">
							<c:forEach items="${questions}" var="question" varStatus="count">
								<div id="testing_answer[${exercise_id}][${count.index + 1}]">
									...
									<c:if test="${question.option1 != null}">
										<label class="fulltest_answer_label"> <input
											type="radio" name="answer[${exercise_id}][${question.id}]"
									...
									<c:if test="${question.option2 != null}">
										<label class="fulltest_answer_label"> <input
											type="radio" name="answer[${exercise_id}][${question.id}]"
											value="2"> <span>${question.option2}</span>
										</label>
									</c:if>
									<c:if test="${question.option3 != null}">
										<label class="fulltest_answer_label"> <input
											type="radio" name="answer[${exercise_id}][${question.id}]"
											value="3"> <span>${question.option3}</span>
										</label>
									</c:if>
									<c:if test="${question.option4 != null}">
										<label class="fulltest_answer_label"> <input
											type="radio" name="answer[${exercise_id}][${question.id}]"
											value="4"> <span>${question.option4}</span>
										</label>
									</c:if>
								</div>
							</c:forEach>
...
<!-- Input Hidden -->
<input id="create_time" type="hidden" value="${create_time}">